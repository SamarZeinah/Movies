...
import styles from './TrendingmoviesItem.module.css';
import { Link } from 'react-router-dom';
const {trendimage,videocontainer,imagetitle,youtubeIcon}=styles;

// ال هدخلهاProps هنا لازم اعرف ال
...
//بعد كدا اديهم للفانكشن بتاعتى
const TrendingmoviesItem=({id,title,poster_path}:TrendingmoviesProps)=>{
    return(
        <>
       
        <div className="d-flex justify-content-between">
        <Link style={{textDecoration:"none",color:"white"}} to={`/MovieDetails/${id}`} className={videocontainer}> 
        <img className={trendimage} src={`https://image.tmdb.org/t/p/w500${poster_path}`}
                    alt={`${title} poster`}  />
